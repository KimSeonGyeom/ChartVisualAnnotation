...
const RANK_INSTRUCTION = 'Please rank the annotations that best help the reader understand the caption, from 1st to 3rd place.';
...
export default function ReviewPage() {
  ...
  const [imageOrder, setImageOrder] = useState({}); // { stimulusId: ['v1','v3','v2'] }
  ...
  // Returns the rank position (1/2/3) assigned to a version, or null
  const getVersionRank = (stimulusId, versionKey) => {
    for (let pos = 1; pos <= 3; pos++) {
      if (responses[`${stimulusId}_rank_${pos}`] === versionKey) return pos;
    }
    return null;
  };

  // Returns available version options for a given rank position
  const getAvailableVersions = (stimulusId, currentPos, order) => {
    const usedVersions = [1, 2, 3]
      .filter(p => p !== currentPos)
      .map(p => responses[`${stimulusId}_rank_${p}`])
      .filter(Boolean);
    return order.filter(v => !usedVersions.includes(v));
  };
  ...
  const validateSubmission = () => {
    ...
    stimuli.forEach(stimulus => {
      ...
      [1, 2, 3].forEach(pos => {
        const key = `${stimulus.id}_rank_${pos}`;
        if (!responses[key]) newErrors[key] = 'Required';
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      await saveReviewData({ responses, trials: stimuli.map(s => s.id), imageOrder });
      navigate('/finish');
    } catch (err) {
        ...
        {stimuli.map((stimulus) => {
          ...
          return (
            <div key={stimulus.id} className="review-trial-section">
              ...
              {/* ── Ranking section (top) ── */}
              <div className="review-ranking-section">
                <p className="review-ranking-instruction">{RANK_INSTRUCTION}</p>
                <div className="review-ranking-positions-row">
                  <div className="review-ranking-positions">
                    {[1, 2, 3].map(pos => {
                      const rankPosKey = `${stimulus.id}_rank_${pos}`;
                      const selectedVersion = responses[rankPosKey] || '';
                      const availableVersions = getAvailableVersions(stimulus.id, pos, order);
                      const ORDINALS = { 1: '1st', 2: '2nd', 3: '3rd' };

                      return (
                        <div key={pos} className={`review-rank-position ${errors[rankPosKey] ? 'has-error' : ''}`}>
                          <span className="review-rank-position-label">{ORDINALS[pos]} place</span>
                          <select
                            className="review-rank-select"
                          ...
                          >
                            ...
                            {order.map((versionKey, idx) => {
                              ...
                              const isAvailable = availableVersions.includes(versionKey);
                              return (isAvailable || selectedVersion === versionKey) ? (
                                <option key={versionKey} value={versionKey}>Image {label}</option>
                              ) : null;
                            })}
                          </select>
                          {errors[rankPosKey] && <span className="review-error-text">{errors[rankPosKey]}</span>}
                        </div>
                      );
                    })}
                  </div>
                  <button
                    className="review-ranking-reset-btn"
                    onClick={() => {
                      setResponses(prev => {
                        const next = { ...prev };
                        [1, 2, 3].forEach(pos => delete next[`${stimulus.id}_rank_${pos}`]);
                        return next;
                      });
                      setErrors(prev => {
                        const next = { ...prev };
                  ...
                  return (
                    <div key={versionKey} className={`review-card ${cardGenerating ? 'review-card-loading' : ''}`}>
                      <div className="review-card-header">
                        <span className="review-card-label">Image {label}</span>
                        {(() => {
                          const rank = getVersionRank(stimulus.id, versionKey);
                          const ORDINALS = { 1: '1st', 2: '2nd', 3: '3rd' };
                          return rank ? <span className="review-rank-badge-card">{ORDINALS[rank]} place</span> : null;
                        })()}
                      </div>
